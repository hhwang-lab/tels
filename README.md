...
- 將個人分數與論文教師樣本平均並列比較。
- 完成後呈現四因素的定義與重要成分；作答過程不揭露個別題目的因素歸屬。
- 報表以發散長條圖呈現四因素與對照平均：中央垂直線是論文教師樣本平均，個人分數高於對照時向右、低於對照時向左，長度代表差距大小。
- 可點選四因素列，或使用鍵盤聚焦後按 Enter／空白鍵；選取後其餘列淡出、選取列與端點會動態強調，資訊卡也會同步更新該因素的定義與重要成分。
- 四因素定義卡在因素名稱旁附上同一份個人結果的迷你發散對照條；點選卡片也能回到主圖聚焦該因素。
- 公開首頁保留中性的「後台管理」入口，管理者驗證後可依教師／非教師分組查看累積回覆，並匯出含基本分組資料的 CSV／JSON；未驗證前不顯示後台資料或功能細節，完成查看後可登出並清除本次頁面中的後台資料。
- 支援列印或另存成 PDF 的個人報表。
- 個人報表可生成 9:16 PNG 圖卡，內含總平均、四因素發散長條圖與論文教師樣本平均對照；可下載保存，或在支援 Web Share 的手機上開啟系統分享選單（例如選擇 Instagram）。
...
管理者安全性：試算表本身維持私有；Apps Script 部署選「以我執行」；一般填答只能呼叫新增回覆，`getAdminData` 只有在管理者代碼正確時才回傳資料。公開首頁只顯示中性的「後台管理」，未驗證前不會顯示後台資料或功能細節；管理者也可使用網站網址後加 `#admin` 的入口，例如 `https://hhwang-lab.github.io/tels/#admin`。GitHub Pages 未設定 Actions secret 時，後台只會顯示「尚未連接雲端」，不會顯示任何瀏覽器累積紀錄。
...
題目來自專案中的 `【定稿】情緒素養量表(35題).docx`；四因素的定義與重要成分依引用論文整理，因素配置與平均數來自 `EL Scale (Accepted Version)_July.26.pdf`，論文報告教師樣本數為 554 人。
