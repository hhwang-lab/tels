...
- 管理者可由專用入口依教師／非教師分組查看累積回覆，並匯出含基本分組資料的 CSV／JSON；公開首頁不顯示後台入口，後台資料必須通過管理者代碼驗證。
- 支援列印或另存成 PDF 的個人報表。
...
管理者安全性：試算表本身維持私有；Apps Script 部署選「以我執行」；一般填答只能呼叫新增回覆，`getAdminData` 只有在管理者代碼正確時才回傳資料。公開首頁不顯示後台導覽；管理者請使用網站網址後加 `#admin` 的專用入口，例如 `https://hhwang-lab.github.io/tels/#admin`。GitHub Pages 未設定 Actions secret 時，後台只會顯示「尚未連接雲端」，不會顯示任何瀏覽器累積紀錄。
