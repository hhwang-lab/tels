...
- 將個人分數與論文教師樣本平均並列比較。
- 完成後以《Manuscript_Jun19.26》表 3 呈現四因素的向度定義與重要成分；作答過程不揭露個別題目的因素歸屬。
- 報表以四瓣圓形面積圖呈現四因素：每個 90° 彩色扇區依 1–6 分由中心向外延伸，固定 6 分的圓形外圈不是論文平均，論文教師樣本平均只以灰綠色虛線弧線顯示。
- 可點選四因素扇區／名稱，或使用鍵盤聚焦後按 Enter／空白鍵；選取後其餘扇區淡出，選取扇區的虛線輪廓會脈動，資訊卡也會更新顯示表 3 的定義與重要成分。
- 四因素定義卡在因素名稱旁附上同一份個人輪廓的迷你四瓣面積圖；點選卡片也能回到主圖聚焦該因素。
- 在「資料後台」依教師／非教師分組顯示累積回覆，並匯出含基本分組資料的 CSV／JSON；後台資料必須通過管理者代碼驗證。
- 支援列印或另存成 PDF 的個人報表。
- 個人報表可生成 9:16 PNG 圖卡，內含總平均、四因素圓形輪廓與論文教師樣本平均對照；可下載保存，或在支援 Web Share 的手機上開啟系統分享選單（例如選擇 Instagram）。
...
題目來自專案中的 `【定稿】情緒素養量表(35題).docx`；四因素的定義與重要成分依 `Manuscript_Jun19.26.docx` 表 3 呈現，因素配置與平均數來自 `EL Scale (Accepted Version)_July.26.pdf`，論文報告教師樣本數為 554 人。
